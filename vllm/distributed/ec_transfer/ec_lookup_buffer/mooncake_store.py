...
class ECMooncakeStore:
    ...
    def get(self, key: str) -> Optional[torch.Tensor]:
        logger.error(
            "Single get operation is not supported. Use batch_get instead.")
    ...
    def _batch_get(self, keys: list[str],
        ...
        try:
            ...
            logger.error("batch_get failed: %s", str(e))
            return [None] * len(keys)
    ...
    def put(self, key: str, tensor: torch.Tensor) -> None:
        logger.error(
            "Single put operation is not supported. Use batch_put instead.")
        raise NotImplementedError("Single put is not supported. "
                                  "Use batch_put([key], [tensor]) instead.")
